...
public class ClientContext {
	 ...
	 * Properties
	 */
	private static final String API_PROTOCAL_KEY = CommonParam.API_PROTOCAL;

	private static final String API_HOST_KEY = CommonParam.API_HOST;

	private static final String API_ORG_KEY = CommonParam.API_ORG;

	private static final String API_APP_KEY = CommonParam.API_APP;

	private static final String APP_CLIENT_ID_KEY = CommonParam.APP_CLIENT_ID;

	private static final String APP_CLIENT_SECRET_KEY = CommonParam.APP_CLIENT_SECRET;

	private static final String APP_IMP_LIB_KEY = CommonParam.APP_IMP_LIB;
...
//			log.warn("Context has been initialized already, skipped!");
		...
		if (INIT_FROM_PROPERTIES.equals(type)) {
			...
			log.error(MessageTemplate.print(MessageTemplate.UNKNOW_TYPE_MSG,
					new String[] { type, "context initialization" }));
...
//		}
		...
		String protocal = API_PROTOCAL_KEY;
		String host = API_HOST_KEY;
		String org = API_ORG_KEY;
		String app = API_APP_KEY;
		String clientId = APP_CLIENT_ID_KEY;
		String clientSecret = APP_CLIENT_SECRET_KEY;
		String impLib = APP_IMP_LIB_KEY;

		if (StringUtils.isBlank(protocal) || StringUtils.isBlank(host)
				|| StringUtils.isBlank(org) || StringUtils.isBlank(app)
				|| StringUtils.isBlank(clientId)
				|| StringUtils.isBlank(clientSecret)
				|| StringUtils.isBlank(impLib)) {
